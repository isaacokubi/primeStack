...
const allowedOrigins = (process.env.CLIENT_URL || 'http://localhost:5173').split(',').map(url => url.trim()).filter(Boolean);
...
const isAllowedOrigin = origin => {
  ...
  try {
    ...
    return hostname.endsWith('.onrender.com') || hostname.endsWith('.vercel.app') || hostname === 'localhost' || hostname === '127.0.0.1';
  } catch { return false; }
};
...
app.use(helmet());
app.use(cors({
  origin(origin, callback) { return isAllowedOrigin(origin) ? callback(null, true) : callback(new Error('CORS origin denied')); },
  credentials: true,
...
app.get('/api/health', (_req, res) => ok(res, { status: 'OK', database: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected' }));
app.get('/health', (_req, res) => ok(res, { status: 'OK', database: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected' }));

app.post('/api/auth/register', async (req, res) => {
  try {
    const name = String(req.body?.name || '').trim(); const email = String(req.body?.email || '').toLowerCase().trim(); const password = String(req.body?.password || '');
...
app.get('/api/auth/me', auth, async (req, res) => { const user = await User.findById(req.user.id); return user ? ok(res, { user: publicUser(user) }) : fail(res, 'User not found', 404); });
app.put('/api/auth/profile', auth, async (req, res) => {
  try {
    const user = await User.findById(req.user.id); if (!user) return fail(res, 'User not found', 404);
    if (req.body.name !== undefined) { const name = String(req.body.name).trim(); if (name.length < 2) return fail(res, 'Name must contain at least 2 characters'); user.name = name; }
    if (req.body.password) { const password = String(req.body.password); if (password.length < 8) return fail(res, 'New password must be at least 8 characters'); user.password = await bcrypt.hash(password, 12); }
    await user.save(); return ok(res, { user: publicUser(user) }, 'Profile updated');
  } catch (error) { return fail(res, error.message, 500); }
});
...
app.get('/api/customer/dashboard', auth, roles('Customer'), async (req, res) => {
  try {
    const user = await User.findById(req.user.id); if (!user) return fail(res, 'User not found', 404);
    const [products, requests] = await Promise.all([Product.find({ published: true }).sort({ featured: -1, createdAt: -1 }).limit(6).lean(), Contact.find({ email: user.email }).sort({ createdAt: -1 }).limit(8).lean()]);
    return ok(res, { user: publicUser(user), stats: { products: products.length, requests: await Contact.countDocuments({ email: user.email }) }, products, requests });
  } catch (error) { return fail(res, error.message, 500); }
});

const siteDefaults = () => ({
  key: 'site', businessType: 'technology', name: 'primeStack', tagline: 'Software that moves your business forward.',
  ...
  email: process.env.SITE_EMAIL || 'hello@primestack.dev', phone: '', logoUrl: '', faviconUrl: '', founderImageUrl: '',
  primaryColor: '#111827', accentColor: '#2563eb', backgroundColor: '#ffffff', textColor: '#111827',
  nav: [{ label: 'Products', path: '/products', visible: true }, { label: 'Services', path: '/services', visible: true }, { label: 'Case Studies', path: '/case-studies', visible: true }, { label: 'Technology', path: '/technology', visible: true }, { label: 'Blog', path: '/blog', visible: true }, { label: 'Careers', path: '/careers', visible: true }, { label: 'About', path: '/about', visible: true }],
  ...
  services: [{ name: 'Custom Software Development', description: 'Business software designed around your workflows.' }, { name: 'Project Delivery', description: 'Planning, coordination and execution from idea to completion.' }],
  technologies: ['React', 'Node.js', 'MongoDB', 'Cloud'], values: ['Quality', 'Transparency', 'Safety', 'Customer Focus'], stats: [{ value: '10+', label: 'Products' }, { value: '50K+', label: 'Users served' }, { value: '99.9%', label: 'Uptime' }, { value: '8+', label: 'Years experience' }], footer: { description: 'We build dependable solutions that create measurable business value.', copyright: 'All rights reserved.' }, seo: {}
});

app.get('/api/site-settings', async (_req, res) => { try { const defaults = siteDefaults(); let settings = await SiteSettings.findOne({ key: 'site' }).lean(); if (!settings) settings = await SiteSettings.create(defaults); return ok(res, { ...defaults, ...settings, sections: { ...defaults.sections, ...(settings.sections || {}) }, home: { ...defaults.home, ...(settings.home || {}) }, pages: { ...defaults.pages, ...(settings.pages || {}) }, footer: { ...defaults.footer, ...(settings.footer || {}) } }); } catch (error) { return fail(res, error.message, 500); } });
app.put('/api/site-settings', auth, roles('Admin', 'Editor'), async (req, res) => { try { const body = { ...req.body, key: 'site' }; delete body._id; delete body.createdAt; delete body.updatedAt; if (typeof body.founderImageUrl === 'string' && body.founderImageUrl.length > 1600000) return fail(res, 'Founder photo is too large after compression. Please choose a smaller image.'); const settings = await SiteSettings.findOneAndUpdate({ key: 'site' }, body, { new: true, upsert: true, runValidators: true, setDefaultsOnInsert: true }).lean(); return ok(res, settings, 'Website settings published'); } catch (error) { return fail(res, error.message, 500); } });

app.get('/api/products', async (req, res) => {
  try {
    const filter = { published: true }; if (req.query.category) filter.category = req.query.category; if (req.query.status) filter.status = req.query.status; if (req.query.featured === 'true') filter.featured = true;
    if (req.query.search) filter.$or = [{ name: { $regex: req.query.search, $options: 'i' } }, { tagline: { $regex: req.query.search, $options: 'i' } }, { description: { $regex: req.query.search, $options: 'i' } }];
    const limit = Math.min(Number(req.query.limit) || 12, 50); const page = Math.max(Number(req.query.page) || 1, 1);
    const [data, total] = await Promise.all([Product.find(filter).sort({ featured: -1, createdAt: -1 }).skip((page - 1) * limit).limit(limit).lean(), Product.countDocuments(filter)]);
    return ok(res, data, 'Products', { page, limit, total, pages: Math.ceil(total / limit) });
  } catch (error) { return fail(res, error.message, 500); }
});
app.get('/api/products/:slug', async (req, res) => { const product = await Product.findOne({ slug: req.params.slug, published: true }).lean(); return product ? ok(res, product) : fail(res, 'Product not found', 404); });
app.post('/api/products', auth, roles('Admin', 'Editor'), async (req, res) => { try { const product = await Product.create({ ...req.body, slug: slugify(req.body.slug || req.body.name) }); return ok(res, product, 'Product created', null, 201); } catch (error) { return fail(res, error.code === 11000 ? 'A product with this slug already exists' : error.message); } });
app.put('/api/products/:id', auth, roles('Admin', 'Editor'), async (req, res) => { try { const product = await Product.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true }); return product ? ok(res, product, 'Product updated') : fail(res, 'Product not found', 404); } catch (error) { return fail(res, error.message); } });
app.delete('/api/products/:id', auth, roles('Admin'), async (req, res) => { const product = await Product.findByIdAndDelete(req.params.id); return product ? ok(res, null, 'Product deleted') : fail(res, 'Product not found', 404); });
...
app.put('/api/contact/:id', auth, roles('Admin'), async (req, res) => { const contact = await Contact.findByIdAndUpdate(req.params.id, { status: req.body.status }, { new: true, runValidators: true }); return contact ? ok(res, contact, 'Updated') : fail(res, 'Inquiry not found', 404); });
...
app.put('/api/admin/users/:id/status', auth, roles('Admin'), async (req, res) => { const status = ['Active', 'Suspended'].includes(req.body.status) ? req.body.status : null; if (!status) return fail(res, 'Invalid status'); const user = await User.findByIdAndUpdate(req.params.id, { status }, { new: true }).select('-password'); return user ? ok(res, user, 'User status updated') : fail(res, 'User not found', 404); });

const seed = async () => {
  if (!mongoose.connection.readyState) return;
  const email = (process.env.ADMIN_EMAIL || 'admin@primestack.dev').toLowerCase();
  if (!await User.exists({ email })) { const password = process.env.ADMIN_PASSWORD || 'ChangeMe123!'; if (process.env.NODE_ENV === 'production' && !process.env.ADMIN_PASSWORD) throw new Error('ADMIN_PASSWORD must be configured in production'); await User.create({ name: process.env.ADMIN_NAME || 'PrimeStack Admin', email, password: await bcrypt.hash(password, 12), role: 'Admin' }); }
