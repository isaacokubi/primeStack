  ...
  return (
    <div className="customerShell">
      <aside className="dashboardSide">
        <Link className="dashboardBrand" to="/"><span className="logo">&lt;/&gt;</span><span>primeStack</span></Link>
        ...
        <div className="dashboardHelp"><strong>Need help?</strong><p>Talk to the primeStack team about your next project.</p><Link to="/contact">Contact team <ArrowRight size={13} /></Link></div>
      ...
      <div className="customerMain">
        ...
        <section className="dashboardSection">
          <div className="dashboardSectionHead"><div><span className="eyebrow">YOUR PRODUCTS</span><h2>Products &amp; services</h2><p>Software available in your primeStack workspace.</p></div><Link to="/products">View catalog <ArrowRight size={16} /></Link></div>
          {products.length ? <div className="dashboardProductGrid">{products.map(product => <Link className="dashboardProduct" key={product._id || product.slug} to={`/products/${product.slug}`}><span className="pill">{product.category || 'Business'} · {product.status || 'Live'}</span><h3>{product.name || 'Product'}</h3><p>{product.tagline || product.description || 'Explore this primeStack product.'}</p><span>View product <ArrowRight size={15} /></span></Link>)}</div> : <div className="dashboardEmpty"><Box /><h3>No products available yet</h3><p>Products published by primeStack will appear here.</p><Link className="btn primary" to="/products">Browse catalog <ArrowRight size={15} /></Link></div>}
        </section>

        <section className="dashboardSection">
          <div className="dashboardSectionHead"><div><span className="eyebrow">ACTIVITY</span><h2>Recent enquiries</h2><p>Keep track of conversations with the primeStack team.</p></div><Link to="/contact">New enquiry <ArrowRight size={16} /></Link></div>
          {requests.length ? <div className="requestList">{requests.map(request => { const status = request.status || 'New'; return <div className="requestItem" key={request._id || `${request.email}-${request.createdAt}`}><div><b>{request.projectType || 'General enquiry'}</b><p>{request.message || 'No message provided.'}</p></div><span className={`status status-${status.toLowerCase().replace(/\s+/g, '-')}`}>{status}</span></div>; })}</div> : <div className="dashboardEmpty"><Mail /><h3>No enquiries yet</h3><p>Have a software idea or business challenge? Start a conversation with the primeStack team.</p><Link className="btn primary" to="/contact">Start a conversation <ArrowRight size={15} /></Link></div>}
        </section>
      </div>
    </div>
  );
