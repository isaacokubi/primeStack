...
const getApiOrigin = () => {
  try { return new URL(apiBaseUrl, window.location.origin).origin; }
  catch { return ''; }
};
...
const normalizeApiData = data => Array.isArray(data) ? data.map(normalizeRecordImages) : (data && typeof data === 'object' ? normalizeRecordImages(data) : data);
...
export const authApi = {
  login: async data => { const response = await api.post('/auth/login', data); saveAuthUser(response.data?.data?.user); window.dispatchEvent(new Event('primeStackAuthChanged')); return response; },
  register: async data => { const response = await api.post('/auth/register', data); saveAuthUser(response.data?.data?.user); window.dispatchEvent(new Event('primeStackAuthChanged')); return response; },
  me: async () => { if (!getStoredAuthUser()) return { data: { success: true, data: { user: null } } }; const response = await api.get('/auth/me'); const user = response.data?.data?.user; if (user) saveAuthUser(user); return response; },
...
export const siteSettingsApi = {
  ...
  update: async data => { const response = await api.put('/site-settings', data); window.dispatchEvent(new Event('primeStackSiteSettingsChanged')); return response; },
};
