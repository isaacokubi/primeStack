...
api.interceptors.response.use(response=>response,error=>{if(error.code==='ECONNABORTED')error.message='Request timed out';return Promise.reject(error)});
...
export const authApi={
 ...
 me:async()=>{try{const r=await api.get('/auth/me');const user=r.data?.data?.user;if(user)saveAuthUser(user);return r}catch(error){if(error.response?.status===401){const stored=getStoredAuthUser();if(stored)return{data:{success:true,data:{user:stored},fromStoredSession:true}}}throw error}},
...
export const siteSettingsApi={get:()=>api.get('/site-settings'),update:data=>api.put('/site-settings',data)};
